...
def plot(sample):
    ...
    for d in [5,10, 15, 20]:
        ...
        for i in range(len(mean)):
            if mean[i] > 0.5:
                if(d==5):
                    plt.axvline(x=i, linestyle=":", color = c[j])
                    plt.text(i, 0.2, r"$n_0=$ %s" %i, color = c[j])
                    j=j+1
                    break
                elif (d==10):
                    plt.axvline(x=i, linestyle=":", color = c[j])
                    plt.text(i, 0.1, r"$n_0=$ %s" %i, color = c[j])
                    j=j+1
                    break
                elif(d==15):
                    plt.axvline(x=i, linestyle=":", color = c[j])
                    ...
                    plt.axvline(x=i, linestyle=":", color = c[j])
                    plt.text(i, 0.4, r"$n_0=$ %s" %i, color = c[j])
                    j=j+1
                    break
        std = np.std(data,axis = 0)
        plt.plot(mean, label = "dimension = %s" %d)
